...
for(cluster in unique(combined_df$cluster)) {
  boxplot(scale(combined_df[combined_df$cluster == cluster, 52:55], center=F, scale=T), main=paste('Cluster', cluster, 'Sums', sep=' '))
}
```
...
par(mar = c(4, 2, 2, 2))
pheatmap(t(scale(do.call(rbind, selected_columns_list), center=F, scale=T)), height=2)
```
...
for (cluster in seq_along(unique(combined_df$cluster))) {
  boxplot(scale(combined_df[combined_df$cluster == cluster, 33:51]), height=1)
}
par(las=0)
```
