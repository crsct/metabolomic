...
#### Exkurs ins andere R-Skript
# Cluster: Partitioning Around Medoids (a more robust version of K-means)

library(cluster)# Heatmaps + Cluster Analysis
library(pheatmap)
pheatmap(logdata,scale = "none", cluster_rows = T, cluster_cols = T)
pheatmap(logdata.pareto,scale = "none", cluster_rows = T, cluster_cols = T)


#### try to fix the pam clara thing 
#### funktioniert ist aber nicht schön:

# colnames(logdata.pareto) = paste("Metabolit_", 1:32, sep = "")

#### 
library(ggplot2)
clara.result <- (clara(logdata.pareto, 5))

autoplot(clara.result, frame = TRUE, frame.type = "norm", loadings = TRUE,loadings.label = FALSE)
